...
function create_pdf(rec, url) {
    var _url = 'https://innovacion.educa.aragon.es/w/api.php?action=query&prop=imageinfo&format=json&iiprop=url&iiurlwidth=200&titles=Archivo:';
    jQuery.ajax({
        dataType: "json",
        url: _url + url,
        async: false,
...
function create_kizoa(rec, url) {
    var _url = url.split('/').pop()
    $(rec).append('<div class="embed-responsive embed-responsive-16by9"> <iframe  src="https://www.kizoa.com/embed-' + _url + '" frameborder = "0" fs="1" allowfullscreen> < /iframe></div > ');
}
...
jQuery(document).ready(
    jQuery.ajax({
        type: "GET",
        url: "https://innovacion.educa.aragon.es/w/api.php",
...
function lineaFormatter(elmt) {
    ...
    switch (link[0].textContent) {
        case "TAC (Tecnologías del Aprendizaje y del Conocimiento)":
            dest.replaceWith('<img src="img/18px-TAC.svg.png" alt="TAC">').show();
            break;
        case "Gestión de las emociones":
            dest.replaceWith('<img src="img/18px-Gestion_de_emociones.svg.png">');
            break;
        case "Metodologías activas":
            dest.replaceWith('<img src="img/18px-Metodologias_activas.svg.png">');
            break;
        case "Compromiso social":
            dest.replaceWith('<img src="img/18px-Compromiso_social.svg.png">');
            break;
        case "Comunicación oral":
            dest.replaceWith('<img src="img/18px-Monstruo_comunicacion_oral.svg.png">');
            break;
            }
}
